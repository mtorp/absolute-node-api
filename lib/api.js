var absolutecoin = require('node-absolutecoin');
...
module.exports = function(){
  ...
  function express_app(){
    ...
    function specialApiCase(method_name, query_parameters){
      ...
      if(method_name == 'sendtoaddress'){
        var secret = md5(query_parameters['addr'] + query_parameters['amt'] + secret_key);
        //console.log(secret);
        if (Object.keys(query_parameters).length == 3){
          for(var parameter in query_parameters){
            if(query_parameters.hasOwnProperty(parameter)){
              var param = query_parameters[parameter];
              switch (parameter){
                ...
                case 'amt':
                  ...
                  break;
                case 'secret':
                  if (query_parameters['secret'] != secret){
                    params = [];
                  } else {
                    params.push(param);
                  }
  ...
  function setAccess(type, access_list){
    ...
    if(type == 'read-only'){
      accesslist.type = 'restrict';
      var restrict_list = ['addmultisigaddress', 'addnode', 'backupwallet', 'createmultisig', 'createrawtransaction', 'encryptwallet', 'importprivkey', 'keypoolrefill', 'lockunspent', 'move', 'sendfrom', 'sendmany', 'sendrawtransaction', 'setaccount', 'setgenerate', 'settxfee', 'signmessage', 'signrawtransaction', 'stop', 'submitblock', 'walletlock', 'walletpassphrasechange'];
      var i=0;
      for(;i<restrict_list.length;i++){
        accesslist[restrict_list[i]] = false;
      }
  ...
  function setWalletDetails(details){
    if('undefined' == typeof details.rpc){
      client = new absolutecoin.Client(details);
    }
    else{
      client = details;
    }
